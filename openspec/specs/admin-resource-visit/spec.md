# admin-resource-visit Specification

## Purpose
admin 文件管理表为每个文件资源提供"在新标签打开静态服务器 URL"的访问入口，支持快速验证上传产物。覆盖 URL 拼接、目录排除、安全属性（`target=_blank` + `rel`）、无障碍标签，以及静态服务器端口在 dev/prod 下的来源（不硬编码）。
